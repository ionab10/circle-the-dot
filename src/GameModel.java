...
public class GameModel implements Cloneable{
	...
	public GameModel clone(){
		...
		GameModel temp;
		try {
			temp = (GameModel) super.clone();
			temp.currentDot = (Point)currentDot.clone();
			temp.model = (int[][])this.model.clone();
		/*
			System.out.println("clone");
            for(int i = 0; i < getSize(); i++){
    		   	for(int j = 0; j < getSize(); j++){
    		   		//temp.model[i][j]= getCurrentStatus(i, j);
    		   		System.out.print(temp.getCurrentStatus(i,j));
    		   	}
    		   	System.out.println();
    		}
            System.out.println();
            */
			System.out.println("Clone: " + temp.getCurrentDot().getX() + "," + temp.getCurrentDot().getY());
            
            
			//temp.currentDot = this.currentDot.clone();
			//temp.model[temp.currentDot.getX()][temp.currentDot.getY()] = 2;
