...
public class GameController implements ActionListener {
    ...
    private LinkedStack<GameModel> gameModelStackUndo = new LinkedStack<GameModel>();
    ...
    public void reset(){
        gameModel.reset();
        while (!gameModelStackUndo.isEmpty()){
        	gameModelStackUndo.pop();
        };
        gameModelStackUndo.push(gameModel.clone());
        gameView.update();
    }
    ...
    public void actionPerformed(ActionEvent e) {
        if (!gameModelStackUndo.isEmpty()){
        	//System.out.println("visible");
        	gameView.buttonUndo.setVisible(true);
        } else {
        	//System.out.println("invisible");
        	gameView.buttonUndo.setVisible(false);
        }
        if (e.getSource() instanceof DotButton) {
            ...
            if (clicked.getText().equals("Quit")) {
                ...
                gameModel = null; // Clearing the memory?
               
              
                //System.out.println("before stack");
                /*for(int i = 0; i < gameModelStackUndo.peek().getSize(); i++){
        		   	for(int j = 0; j < gameModelStackUndo.peek().getSize(); j++){
        		   		System.out.print(gameModelStackUndo.peek().getCurrentStatus(i, j));
        		   	}
        		   	System.out.println();
        		}
                System.out.println(gameModelStackUndo.peek().getCurrentDot().getX() + "," + gameModelStackUndo.peek().getCurrentDot().getY());
            	*/
                System.out.println("Before popping: " + gameModelStackUndo.peek().getCurrentDot().getX() + "," + gameModelStackUndo.peek().getCurrentDot().getY());
                
            	gameModelStackUndo.pop();
            	gameModel = gameModelStackUndo.peek();
                ...
                System.out.println("Peeked from stack: " + gameModelStackUndo.peek().getCurrentDot().getX() + "," + gameModelStackUndo.peek().getCurrentDot().getY());
                
            	//gameModel.setCurrentDot(gameModel.getCurrentDot().getX(), gameModel.getCurrentDot().getY());
                gameView.update(gameModelStackUndo.peek());
    ...
    private void oneStep(){
        ...
        System.out.println("Pushed to stack: " + gameModelStackUndo.peek().getCurrentDot().getX() + "," + gameModelStackUndo.peek().getCurrentDot().getY());
    }
